...
class GiveawayPlugin(commands.Cog):
    ...
    async def generate_giveaway(self):
        giveaway_item = "20 Typical Tokens"
        giveaway_winners = 2
        giveaway_time = time.time() + 86400
        channel = self.bot.get_channel(888237681694351401)
...
def setup(bot):
    g = GiveawayPlugin(bot)
    g.scheduler = AsyncIOScheduler()
    g.scheduler.add_job(g.generate_giveaway, CronTrigger(hour=0, minute=0, second=0))
    g.scheduler.start()
    bot.add_cog(g)
